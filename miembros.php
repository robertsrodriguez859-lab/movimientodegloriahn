...
<div class="app">
  ...
  <header class="top">
    <div class="title-block">
      <h1>Miembros</h1>
      <div class="subtitle">Libro de la congregación en formato de listado</div>
    </div>
  ...
  <div class="menu-hamburguesa-container">
    ...
    <div class="menu-desplegable" id="menuDesplegable">
      <a href="miembros.php" class="menu-item actual">Miembros</a>
  ...
  <button class="fab" id="btnNuevo" title="Añadir miembro">+</button>
</div>

<!-- MODAL REGISTRO -->
<div class="overlay" id="overlay" style="display:none;">
  <div class="modal">
    <h2 id="modalTitulo">Registrar Nuevo Miembro</h2>
    <form id="form">
      <div class="form-grid">
        <label class="full">Nombre completo
          <input type="text" id="f_nombre" required>
        </label>
        <label>Cargo / Rol ministerial
          <select id="f_cargo" required>
...
<script type="module">
  ...
  function render(){
    ...
    if(items.length === 0){
      lista.innerHTML = `<div class="vacio">Aún no hay registros. Pulsa "+" para añadir el primero.</div>`;
      return;
    }
    ...
    lista.innerHTML = items.map(r => `
      <div class="card">
        <div class="info">
          ...
          <div class="detalle">
            <span class="badge">Cargo: ${escapeHtml(r.cargo || 'Miembro')}</span>
  ...
  document.getElementById('btnNuevo').addEventListener('click', () => { editandoId = null; document.getElementById('modalTitulo').textContent = 'Registrar Nuevo Miembro'; document.getElementById('form').reset(); document.getElementById('f_cargo').value = 'Miembro'; abrirModal(); });
  ...
  function abrirEdicion(id){
    ...
    document.getElementById('f_cargo').value = r.cargo || 'Miembro';
  ...
  document.getElementById('form').addEventListener('submit', async (e) => {
    ...
    if(editandoId){
      ...
      registros.push({ id: docRef.id, ...datos });
    }
    cerrarModal();
    render();
  });
