...
<title>Jóvenes Contra Cultura - Ministerio Internacional Movimiento de Gloria</title>
<style>
...
<div class="app">
  ...
  <header class="top">
    <div class="title-block">
      <h1>Jóvenes Contra Cultura</h1>
      <div class="subtitle">Control y registro de la juventud</div>
    </div>
  ...
  <div class="menu-hamburguesa-container">
    ...
    <div class="menu-desplegable" id="menuDesplegable">
      ...
      <a href="jovenes_contra_cultura.php" class="menu-item actual">Jóvenes Contra Cultura</a>
  ...
  <div id="contenido" style="display:none;">
    <div class="toolbar"><input type="text" id="buscar" placeholder="Buscar joven..."></div>
    <div class="lista" id="lista"></div>
  </div>
  <div class="aviso">Los datos se sincronizan directamente con Firebase Firestore.</div>
  <button class="fab" id="btnNuevo" title="Añadir joven">+</button>
</div>
<div class="overlay" id="overlay" style="display:none;">
  <div class="modal">
    <h2 id="modalTitulo">Registrar Joven</h2>
    <form id="form">
      <div class="form-grid">
        <label class="full">Nombre completo<input type="text" id="f_nombre" required></label>
        <label>Cargo / Rol
          <select id="f_rol" required>
...
<script type="module">
  ...
  const COLECCION = 'jovenes_contra_cultura';
    ...
    lista.innerHTML = items.map(r => `
      <div class="card">
        <div class="info">
          ...
          <div class="detalle">
            <span class="badge">Rol: ${escapeHtml(r.rol || 'Joven')}</span>
  ...
  document.getElementById('btnNuevo').addEventListener('click', () => { editandoId = null; document.getElementById('modalTitulo').textContent = 'Registrar Joven'; document.getElementById('form').reset(); abrirModal(); });
  document.getElementById('btnCancelar').addEventListener('click', cerrarModal);
  document.getElementById('buscar').addEventListener('input', render);
  function abrirEdicion(id){
    const r = registros.find(x => x.id === id); if(!r) return;
    editandoId = id;
    document.getElementById('modalTitulo').textContent = 'Editar Joven';
    document.getElementById('f_nombre').value = r.nombre || '';
    document.getElementById('f_rol').value = r.rol || 'Joven';
  ...
  document.getElementById('form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const datos = { nombre: document.getElementById('f_nombre').value.trim(), rol: document.getElementById('f_rol').value.trim(), telefono: document.getElementById('f_telefono').value.trim() };
