...
<title>Visitantes - Ministerio Internacional Movimiento de Gloria</title>
<style>
  ...
  *{box-sizing:border-box;}
  ...
  input[type="text"],select,input[type="tel"],input[type="date"],textarea{font-family:'Karla',sans-serif;font-size:0.92rem;border:1px solid var(--rule);border-radius:6px;padding:10px 14px;background:var(--bg-card);color:var(--ink);width:100%;}
  ...
  /* Estilo de Tarjetas Visitantes */
  .card-visitante{background:var(--bg-card);border:1px solid var(--rule);border-radius:var(--radius);padding:20px;margin-bottom:16px;box-shadow:0 2px 6px rgba(0,0,0,0.02);position:relative;}
  .card-visitante-header{display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:6px;}
  .card-visitante-title{font-family:'Fraunces',serif;font-size:1.25rem;font-weight:600;color:var(--wine-dark);margin:0;}
  .card-visitante-meta{font-size:0.85rem;color:var(--ink-soft);display:flex;gap:16px;flex-wrap:wrap;align-items:center;margin-bottom:12px;}
  .card-actions{display:flex;gap:8px;align-items:center;}
  
  .badge-interes{display:inline-block;font-size:0.68rem;text-transform:uppercase;letter-spacing:0.04em;color:var(--gold);border:1px solid var(--gold);border-radius:4px;padding:2px 8px;font-family:'IBM Plex Mono',monospace;font-weight:600;}
  .badge-enterado{display:inline-block;font-size:0.68rem;text-transform:uppercase;letter-spacing:0.04em;color:var(--ink-soft);border:1px solid var(--rule);border-radius:4px;padding:2px 8px;font-family:'IBM Plex Mono',monospace;}

  .notas-section{border-top:1px dashed var(--rule);padding-top:12px;margin-top:10px;font-size:0.85rem;color:var(--ink-soft);}
  ...
  /* Modal idéntico al diseño requerido */
  ...
  .modal{background:var(--bg-card);border-radius:var(--radius);padding:24px;max-width:540px;width:100%;box-shadow:0 12px 40px rgba(0,0,0,0.25);max-height:90vh;overflow-y:auto;}
  .modal h2{font-family:'Fraunces',serif;color:var(--wine-dark);margin-top:0;margin-bottom:20px;font-size:1.4rem;}
  
  .form-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;}
  .form-grid .full{grid-column:1/-1;}
  .form-grid label{display:flex;flex-direction:column;font-size:0.72rem;font-weight:700;color:var(--ink-soft);gap:5px;text-transform:uppercase;letter-spacing:0.04em;}
...
<div class="app">
  ...
  <header class="top">
    <div class="title-block">
      <h1>Visitantes</h1>
      <div class="subtitle">Control y seguimiento de visitantes</div>
    </div>
  ...
  <div class="menu-hamburguesa-container">
    ...
    <div class="menu-desplegable" id="menuDesplegable">
      ...
      <a href="visitantes.php" class="menu-item actual">Visitantes</a>
  ...
  <div id="contenido" style="display:none;">
    <div class="toolbar"><input type="text" id="buscar" placeholder="Buscar visitante por nombre, teléfono, invitado por..."></div>
    <div id="listaVisitantes"></div>
  </div>
  
  <div class="aviso">Los datos se sincronizan directamente con Firebase Firestore.</div>
  <button class="fab" id="btnNuevo" title="Registrar Visitante">+</button>
</div>

<!-- Modal Registro / Edición Exacto -->
<div class="overlay" id="overlay" style="display:none;">
  <div class="modal">
    <h2 id="modalTitulo">Nuevo registro — Visitantes</h2>
    <form id="form">
      <div class="form-grid">
        ...
        <label>Invitado por
          <input type="text" id="f_invitado_por">
        </label>
        
        <label>¿Cómo se enteró?
          <select id="f_como_entero">
            <option value="">Seleccionar...</option>
            <option value="Redes Sociales">Redes Sociales</option>
            <option value="Un Amigo / Familiar">Un Amigo / Familiar</option>
            <option value="Casa de Paz">Casa de Paz</option>
            <option value="Publicidad">Publicidad</option>
            <option value="Otro">Otro</option>
          </select>
        </label>
        
        <label class="full">¿Le interesa seguir asistiendo?
          <select id="f_interes">
        ...
        <label class="full">Notas
          <textarea id="f_notas" rows="3"></textarea>
        </label>
      </div>
...
<script type="module">
  ...
  function render(){
    ...
    if(items.length === 0){ 
      contenedor.innerHTML = `<div class="vacio">No hay registros de visitantes.</div>`; 
      return; 
    }
    
    contenedor.innerHTML = items.map(r => `
      <div class="card-visitante">
        <div class="card-visitante-header">
          <div>
            <h3 class="card-visitante-title">${escapeHtml(r.nombre || '')}</h3>
            <div class="card-visitante-meta">
              <span>📞 ${escapeHtml(r.telefono || 'Sin teléfono')}</span>
              <span>📅 ${escapeHtml(r.fecha || 'Sin fecha')}</span>
              <span>👤 Invitado por: <strong>${escapeHtml(r.invitado_por || 'N/D')}</strong></span>
            </div>
          </div>
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:8px;">
          ${r.como_entero ? `<span class="badge-enterado">Medio: ${escapeHtml(r.como_entero)}</span>` : ''}
          <span class="badge-interes">¿Sigue asistiendo?: ${escapeHtml(r.interes || 'Sí')}</span>
        </div>
        ${r.notas ? `<div class="notas-section"><strong>Notas:</strong> ${escapeHtml(r.notas)}</div>` : ''}
      </div>`).join('');
  ...
  document.getElementById('btnNuevo').addEventListener('click', () => { 
    editandoId = null; 
    document.getElementById('modalTitulo').textContent = 'Nuevo registro — Visitantes'; 
  ...
  function abrirEdicion(id){
    ...
    editandoId = id;
    document.getElementById('modalTitulo').textContent = 'Editar Visitante';
    ...
    document.getElementById('f_como_entero').value = r.como_entero || '';
    document.getElementById('f_interes').value = r.interes || 'Sí';
  ...
  document.getElementById('form').addEventListener('submit', async (e) => {
    ...
    const datos = { 
      ...
      invitado_por: document.getElementById('f_invitado_por').value.trim(), 
      como_entero: document.getElementById('f_como_entero').value, 
      interes: document.getElementById('f_interes').value, 
